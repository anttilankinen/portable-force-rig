  ...
  render() {
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <Input type="number" disabled={!count} loading={!ready} onChange={this.handleChange} placeholder='Enter weight (N)' value={weight}/> {
          !started && <button className="ui teal button" onClick={this.start}>
              <i className="sync icon"></i>Calibrate
            </button>
        }
        {
          started && <button className="ui teal button" onClick={this.end} disabled={!ready}>
              <i className="stop icon"></i>End
            </button>
        }
        {
          started && <button className="ui teal button" onClick={this.run} disabled={!ready}>
